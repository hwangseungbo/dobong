...
const allowedOrigins = ['http://localhost:3000', 'http://192.168.0.15:3000', 'http://192.168.0.41:3000'];
app.use(
    cors({
...
app.get('/', async (req, res) => {
    let hash = await bcrypt.hash('csa13579!', 10);
    let result = await Users.findOne({ username: 'admin' });
    if (!result) {
        await Users.create({ username: 'admin', password: hash });
    }
...
app.post('/logout', (req, res) => {
    req.logout((err) => {
        ...
        req.session.destroy((err) => {
            if (err) {
                return res.status(500).json({ message: '세션 파기 실패' });
            }
